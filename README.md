...
├── fastenaiting/         # standalone tool: photo -> AI-identified fastener,
│                         # 12mm-tape printable label, Lowe's/HD/Amazon links.
│                         # Also served at fastenaiting.com/ (custom domain).
...
- Some `/api` paths are intentionally **public** (no auth) — currently just
  `/api/fastenaiting/*`. Add path prefixes to the `PUBLIC_PREFIXES` list in
  `functions/api/_middleware.js` to allow more.

## fastenaiting

The `/fastenaiting/` tool identifies fasteners from photos using Anthropic's
vision API and generates a printable 12&nbsp;mm label + Lowe's / Home Depot /
Amazon buy links.

**Enable the identifier** by setting a Pages **secret** with the Anthropic
API key:
...
If the secret isn't set, users can still paste their own key in the tool's
Settings dialog (kept in browser localStorage only) or use the manual entry
form without AI.

**Custom domain (fastenaiting.com):** add it as a custom domain on this same
Pages project — the `_redirects` file already rewrites bare visits at that
host into `/fastenaiting/`.

**Behavioral spec + lock (Bantay):** `fastenaiting/fastenaiting.aide` is the
tool's behavioral contract — CUJs, scenarios, invariants, constraints,
foundations, wisdom, relationships. `fastenaiting/aide.lock` is a
deterministic, kind-grouped, per-entity-hashed snapshot of that aide,
regenerated by `scripts/aide-lock.mjs`. Every aide edit must land with a
matching lock update:

```sh
npm run lock              # regenerate every aide.lock in the repo
npm run lock fastenaiting/fastenaiting.aide   # or just one
```

The lock's git diff is the auditable record of what changed in the spec.
Future CI will reject PRs where the aide and lock disagree, or where code
changes trace back to no aide edit.
